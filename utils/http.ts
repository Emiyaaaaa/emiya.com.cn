import type {
	...
	RouteKey,
	RouteString,
	ServerSideAPIInterface
} from "@/server/route";

export type APIResult<T extends RouteKey> = PromiseReturnType<
	ServerSideAPIInterface[T]
>;

export async function postAPI<T extends RouteKey>(
	api: RouteString<T>,
	...data: RemoveTypeFormArray<
		Parameters<ServerSideAPIInterface[T]>,
		RequestHooks
	>
): Promise<APIResult<T>> {
	...
	return fetch(url, {}).then((res) => {
		if (res.status !== 200) {
			return Promise.reject(
				new Error(`GET ${url} error, code: ${res.status} ${res.statusText}`)
			);
		}
		return res.json().then((data) => data.data);
	});
